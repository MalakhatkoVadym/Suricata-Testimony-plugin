 ...
 */
void TmModuleReceiveTestimonyRegister (int slot)
{
    tmm_modules[slot].name = "ReceiveTestimony";
    tmm_modules[slot].ThreadInit = ReceiveTestimonyThreadInit;
    tmm_modules[slot].PktAcqLoop = ReceiveTestimonyLoop;
    tmm_modules[slot].ThreadExitPrintStats = ReceiveTestimonyThreadExitStats;
    tmm_modules[slot].PktAcqBreakLoop = ReceiveTestimonyBreakLoop;
    tmm_modules[slot].ThreadDeinit = ReceiveTestimonyThreadDeinit;
 ...
 */
void TmModuleDecodeTestimonyRegister (int slot)
{
    tmm_modules[slot].name = "DecodeTestimony";
    tmm_modules[slot].ThreadInit = DecodeTestimonyThreadInit;
    tmm_modules[slot].Func = DecodeTestimony;
    tmm_modules[slot].ThreadDeinit = DecodeTestimonyThreadDeinit;
...
static TmEcode ReceiveTestimonyLoop(ThreadVars *tv, void *data, void *slot)
{
    ...
    while (ttv->running) {
        ...
        if (res < 0) {
            SCLogError(SC_ERR_PLUGIN, "testimony_get_block(): %s, %s",
        ...
        if (res < 0) {
            SCLogError(SC_ERR_PLUGIN, "testimony_return_block(): %s, %s",
                    testimony_error(ttv->t),
                    strerror(-res));
            StatsSyncCountersIfSignalled(tv);
            SCReturnInt(TM_ECODE_FAILED);
...
static TmEcode ReceiveTestimonyThreadInit(ThreadVars *tv, const void *initdata, void **data)
{
    ...
    if (res < 0) {
        SCLogError(SC_ERR_PLUGIN, "testimony_connect(): %s", strerror(-res));
        tconfig->DerefFunc(tconfig);
        SCReturnInt(TM_ECODE_FAILED);
    }
    ...
    if (res < 0) {
        SCLogError(SC_ERR_PLUGIN, "testimony_init(): %s, %s",
                testimony_error(ttv->t),
                strerror(-res));
